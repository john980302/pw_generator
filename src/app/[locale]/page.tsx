...
function StrengthCharacter({ strength, t }: { strength: string; t: ReturnType<typeof useTranslation>["t"] }) {
  if (strength === "Very Strong") {
    return (
      <div className="flex flex-col items-center gap-2">
        ...
        <span className="text-emerald-600 dark:text-emerald-400 font-bold text-sm">{t.home.strength.label.veryStrong}</span>
      </div>
    );
  }
  ...
  if (strength === "Strong") {
    return (
      <div className="flex flex-col items-center gap-2">
        ...
        <span className="text-blue-600 dark:text-blue-400 font-bold text-sm">{t.home.strength.label.strong}</span>
      </div>
    );
  }
  ...
  if (strength === "Medium") {
    return (
      <div className="flex flex-col items-center gap-2">
        ...
        <span className="text-yellow-600 dark:text-yellow-400 font-bold text-sm">{t.home.strength.label.medium}</span>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      ...
      <span className="text-red-600 dark:text-red-400 font-bold text-sm">{t.home.strength.label.weak}</span>
    </div>
  );
}
...
export default function Home() {
  ...
  const getStrengthLabel = (label: string) => {
    if (label === "Very Strong") return t.home.strength.veryStrong;
    if (label === "Strong") return t.home.strength.strong;
    if (label === "Medium") return t.home.strength.medium;
    return t.home.strength.weak;
  };

  const strength = getPasswordStrength();

  const optionLabels: [keyof typeof options, string, string, string][] = [
    ["uppercase", t.home.options.uppercase, "A-Z", "🔤"],
    ["lowercase", t.home.options.lowercase, "a-z", "🔡"],
    ["numbers", t.home.options.numbers, "0-9", "🔢"],
    ["symbols", t.home.options.symbols, "!@#", "✨"],
  ];
        ...
        <div className="rounded-3xl border border-slate-200 bg-white/80 p-6 shadow-2xl backdrop-blur-sm dark:border-slate-800 dark:bg-slate-900/80 md:p-8">
            ...
            <div className="flex-shrink-0">
              {password ? (
                <div className="animate-in fade-in zoom-in duration-300">
                  <StrengthCharacter strength={strength.label} t={t} />
                </div>
              ) : (
                <div className="flex flex-col items-center gap-2">
                  ...
                  <span className="text-slate-400 dark:text-slate-500 text-sm">{t.home.generatePassword}</span>
                </div>
              )}
            </div>
          ...
          <div className="mb-6 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
              {optionLabels.map(([key, label, hint, emoji]) => (
                <div
                  key={key}
                  role="button"
                  tabIndex={0}
